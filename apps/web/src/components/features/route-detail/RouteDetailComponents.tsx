...
export function RouteJourneyTimeline({ route }: { route: Route }) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border mb-6 text-slate-900">
      <h2 className="text-lg font-semibold mb-6 text-slate-900">Journey Details</h2>
      <div className="relative flex items-start gap-8">
        ...
        <div className="flex-1 space-y-12">
          <div className="flex justify-between">
            <div>
              <p className="text-xl font-bold text-slate-900">{format(depDate, 'HH:mm')}</p>
              <p className="text-sm text-gray-500">{format(depDate, 'EEE, dd MMM')}</p>
              <p className="font-medium mt-2 text-slate-900">{route.departureLocation}</p>
            </div>
          </div>
          <div className="flex justify-between">
            <div>
              <p className="text-xl font-bold text-slate-900">{format(arrDate, 'HH:mm')}</p>
              <p className="text-sm text-gray-500">{format(arrDate, 'EEE, dd MMM')}</p>
              <p className="font-medium mt-2 text-slate-900">{route.arrivalLocation}</p>
            </div>
...
export function RouteDetailTabs({ route }: { route: Route }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border overflow-hidden text-slate-900">
      ...
      <div className="p-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {route.amenities.map((amenity) => (
            <div key={amenity.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 text-slate-900">
